<div class="admin-container__content">
    <h2>Изменение блюда</h2>
    <form class="quiz-add-form" action="{{route('admin.products.addData', ['action'=>'edit', 'id'=>$product->id])}}" method="post">
        <div class="row">
            <div class="admin-container__col">
                <x-radio class="" type="checkbox" name="isActive" id="1" checked="{{$product->is_active}}">Активность</x-radio>
                <x-radio class="" type="checkbox" name="isNew" id="2" checked="{{$isNew}}">Новинка</x-radio>
                @if(!$categories->isEmpty())
                    <div class="item" style="display:flex; align-items: center; gap:20px; width: 100%">
                        <div>Категория*:</div>
                        <select name="category" id="coupon">
                ...
                @endif
                <div>
                    <x-input class="validate" name="title" type="text" value="{{$product->title}}">Название*</x-input>
                    <div class="input-error"></div>
                </div>
                <div>
                    <x-input class="validate" name="description" type="text" value="{{$product->description}}">Описание*</x-input>
                    <div class="input-error"></div>
                </div>
                <div class="admin-container__row">
                    <div class="item">
                        <x-input class="validate number" name="weight" type="text"  value="{{$product->weight}}">Вес(г)*</x-input>
                        <div class="input-error"></div>
                    </div>
                    <div class="item">
                        <x-input class="validate number" name="price" type="text" value="{{$product->price}}">Цена(руб)*</x-input>
                        <div class="input-error"></div>
                    </div>
                    @if(!$sales->isEmpty())
                        <div class="item" style="display:flex; align-items: center; gap:20px; width: 50%">
                            <select name="sale" id="coupon">
                                <option value="">Выберите скидку</option>
                                @foreach($sales as $sale)
                                    <option value="{{$sale->id}}"
                                        {{($product->sale_id === $sale->id) ? "selected" : ''}}
                                    >
                                        {{$sale->title}}</option>
                                @endforeach
                            </select>
                ...
                <div class="acent acent--l">Энергетическая ценность</div>
                <div class="admin-container__row">
                    <div class="item">
                        <x-input class="validate number" name="protein" type="text" value="{{$product->attribute->protein}}">Белки*</x-input>
                        <div class="input-error"></div>
                    </div>
                    <div class="item">
                        <x-input class="validate number" name="fats" type="text" value="{{$product->attribute->fats}}">Жири*</x-input>
                        <div class="input-error"></div>
                    </div>
                    <div class="item">
                        <x-input class="validate number" name="carbs" type="text" value="{{$product->attribute->carbs}}">Углеводы*</x-input>
                        <div class="input-error"></div>
                    </div>
                    <div class="item">
                        <x-input class="validate number" name="calories" type="text" value="{{$product->attribute->calories}}">кКал*</x-input>
                        <div class="input-error"></div>
                    </div>
                </div>
