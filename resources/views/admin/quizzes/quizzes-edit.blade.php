<div class="admin-container__content">
    <h2>Изменение квиза</h2>
    <form class="quiz-add-form" action="{{route('admin.quizzes.addData', ['action'=>'edit','id'=>$quiz->id])}}" method="post">
        <div class="row">
            <div class="admin-container__col admin-container__col--l">
                <p>{{$quiz->isActive}}</p>
                <x-radio class="" type="checkbox" name="isActive" id="1" checked="{{$quiz->is_active}}">Активен</x-radio>
                <div>
                    <x-input class="validate" name="title" type="text" value="{{$quiz->title}}">Название*</x-input>
                    <div class="input-error"></div>
                </div>
                <div>
                    <x-input class="validate" name="description" type="text" value="{{$quiz->description}}">Описание*
                    </x-input>
                    <div class="input-error"></div>
                </div>
                ...
                <div>
                    ...
                    <select name="coupon" id="coupon">
                        <option value="{{$quiz->coupon->id}}" selected>{{$quiz->coupon->code}}</option>
                        @if(!$coupons->isEmpty())
            ...
            <div class="admin-container__col admin-container__col--s">
                <div class="input-file">
                    ...
                    <label class="input-file__label">
                        ...
                        <input id="image-input" name="image" type="file" value="{{$quiz->img_path}}"/>
                    </label>
                </div>
